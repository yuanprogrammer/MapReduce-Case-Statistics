...
public class PhoneMain {
    public static void main(String[] args) {
        try {
            ...
            Job job = Job.getInstance(conf, "PhoneMain");//创建一个job作业
            job.setJarByClass(PhoneMain.class);//设置入口类
...
//            job.setNumReduceTasks(0);
            ...
            System.out.println(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date()) + (result ? "京东手机数据（phone）统计完毕！！！" : "统计失败！！！"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
